      ...
      <View style={{marginTop:20}}>
        {categoryData?.CategoryItems?.length> 0 ? categoryData?.CategoryItems?.map((item,index)=>(
            ...
            <View key={index} style={{display:"flex",flexDirection:"row",
                ...
                <Image source={{uri:item.image}} style={{width:90,height:90,borderRadius:15}}/>
                <View style={{flex:1,marginLeft:10}}>
                    ...
                    <Text style={{fontFamily:"outfit-bold",fontSize:20,marginLeft:10}}>{item.cost}₹</Text>
            </View>
            {categoryData?.CategoryItems.length-1!=index&&
            <View style={{borderWidth:0.5,marginTop:7,borderColor:colors.GRAY}}>
            </View>}
            </>
        )):
